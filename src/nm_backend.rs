...
pub fn list_networks() -> Vec<Network> {
    ...
    for line in stdout.lines() {
        // nmcli -t escapes ':' with '\:' and '\' with '\\'
        // We need a smart split that respects escapes
...
fn get_active_ssid() -> Option<String> {
    ...
    String::from_utf8_lossy(&output.stdout)
        ...
        .map(|l| {
            // Again, need to handle escapes
            let mut ssid = String::new();
            let mut escaped = false;
            let content = &l[4..]; // skip "yes:"
...
pub fn list_vpns() -> Vec<VpnConnection> {
    ...
    // 1. Get NMCLI VPNs (wireguard, vpn, etc)
...
pub fn toggle_vpn(name: &str, vpn_type: &str, enable: bool) -> bool {
    if vpn_type == "warp" {
        ...
        // Can't reliably toggle without knowing how it was started, usually handled via app
        false
    } else {
...
pub fn get_network_info(ssid: &str) -> String {
    ...
    // Fallback if not connected or saved
...
pub struct NetworkConfig {
    ...
    pub priority: i32,
    pub dns: String,
    pub ip4_method: String, // "auto", "manual", "shared", "disabled"
}
...
pub fn connect_to_wifi(ssid: &str, password: Option<&str>) -> bool {
    if let Some(pass) = password {
        ...
        // Try bringing up existing connection
        let mut cmd = Command::new("nmcli");
        ...
        // If 'up' failed, try 'dev wifi connect' without password (might use secrets agent)
